# Creating Professional Suno Style Tags

I will need background music for my narratives. First, I want you to research the best methodologies for constructing the most effective Suno tags.

I have a Suno account so I own the copyright to all my music.

1. The tag should begin with [quick intro, 3 seconds max] and end with [quick outro, 3 seconds max].
2. I want an 80's Arcade, SNES chipset sound, but I will also change the themes and moods for these soundtracks. You will need to ask me about these. These are variable so I will swap and switch them often.
3. A little bit of "orchestra" type instruments is ok, but I don't want this to override the video game vibe.
4. I want each sound track to give the listener a sense of encouragement and hope. Something that gets them away, but is not ridiculously fast.
5. Super Important! I don't want the background music to drown out the narration, so anything that can slightly fade after the intro for the narration, and then fade back in after the narration is best!

When I type the command: `>>> SUNO`, you will know I am ready to make the Suno style tags that fit this instruction file.
